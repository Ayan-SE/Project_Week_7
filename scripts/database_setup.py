...
def insert_data(engine, cleaned_data):
    """ Inserts cleaned Telegram data into PostgreSQL database. """
    try:
        # Convert NaT timestamps to None (NULL in SQL)
        cleaned_data["message_date"] = cleaned_data["message_date"].apply(lambda x: None if pd.isna(x) else str(x))
        ...
        logging.info(f"✅ {len(cleaned_data)} records inserted into PostgreSQL database.")
    except Exception as e:
